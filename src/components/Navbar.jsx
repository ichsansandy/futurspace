...
export default function Navbar() {
  ...
  return (
    <div className="relative flex items-center justify-between transition-all">
      ...
      <nav
        className={`${
          isOpen ? 'flex translate-y-0 z-10' : 'flex scale-0 -translate-y-52 web:flex web:scale-100 web:translate-y-0'
        } absolute items-center min-h-screen web:min-h-fit left-0 top-[100%] w-full box-border transition-all backdrop-blur-xl  web:bg-transparent flex-col web:static web:flex-row web:justify-end gap-4 md:gap-5 xl:gap-14`}
      >
        {menus.map((menu) => (
          <NavLink
            key={menu.href}
